...
nets = ['RegNetX_200MF', 'Resnet18','Resnet34', 'VGG19', 'SimpleDLA']
# nets = ['EfficientNetB0', 'MobileNet', 'SENet18', 'ShuffleNetV2']
...
#                     for batch in batch_size:
#                         for weight in weight_decay:
#                             os.system(f"python main.py --dir {'finalgraph/'+ str(net) + '_' + str(lr) + '_' + str(batch) + '_' + str(weight) + '_' + str(data) + '/' + str(seed) } --training_epochs 200 --optimizer {opt} --weight_decay {weight} --lr {lr} --batch_size {batch} --skip_plot 10 --seed {seed} --net {net} --data_choose {data} --task adagrad --scheduler step&")
#                             time.sleep(1500)
