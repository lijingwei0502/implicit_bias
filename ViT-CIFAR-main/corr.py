...
root = 'region_count.txt'
data = np.genfromtxt(root)

data = data[data[:, 3] != 1e-3]  # 过滤掉 region count 为 0 的数据
...
# 根据最后三列排序
sorted_data = np.sort(structured_data, order=['col3', 'col4', 'col5'])

# 使用 numpy 的 split 函数根据最后三列分组
unique_keys, indices = np.unique(sorted_data[['col3', 'col4', 'col5']], return_index=True, axis=0)
...
for index in indexs:
    ...
    y_current = averaged_data[:, 0]-averaged_data[:, 1]
    ...
    plt.title(f'Correlation of Vision Transformer : {correlation:.2f}', fontsize=34)
    root = 'a.png'
