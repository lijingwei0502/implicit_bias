...
for net in nets:
    ...
    # 筛选出第25列=0.1，第26列=256的数据
    averaged_data = averaged_data[(averaged_data[:, 25] == 0.1) & (averaged_data[:, 26] == 256)]

    # 筛选出第24列小于0.001的数据
    averaged_data = averaged_data[averaged_data[:, 24] < 0.001]
    ...
    for index in indexs:
        ...
        plt.title(f'Correlation: {correlation:.2f}', fontsize=34)
        root = str(net) + '.png'
