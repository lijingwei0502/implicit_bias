...
# 在进行排序之前，确保变量列是浮点数类型
averaged_data[:, -2] = averaged_data[:, -2].astype(float)
averaged_data[:, -6] = averaged_data[:, -6].astype(float)

batch_sizes = [256, 512, 1024]
for batch in batch_sizes:
    m1 = averaged_data[averaged_data[:, -1] == batch]
    l1 = m1[:, -2]  # 倒数第二列：learning rate
    r1 = m1[:, -6]  # 倒数第六列：region数

    # 按学习率升序排序
    sorted_indices = np.argsort(l1)
    l1 = l1[sorted_indices]
    r1 = r1[sorted_indices]

    learning_rates = np.round(l1, decimals=5)

    # 创建一个新的等距横坐标映射
    # 例如，对于learning rate，您可以创建一个映射：{0.001: 1, 0.01: 2, 0.1: 3}
    learning_rate_mapping = {0.001: 1,0.005:2, 0.01:3,0.05:4, 0.1:5}

    # 将learning rate的值映射到新的等距值
    mapped_learning_rates = np.array([learning_rate_mapping[lr] for lr in learning_rates])

    plt.plot(mapped_learning_rates, r1, label=f'batch size={batch}', marker=notations[cnt], markersize=10, linewidth=3)
    cnt += 1
...
# 添加标题和轴标签
plt.title('Region Counts v.s. Hyperparameters', fontsize=22)
plt.xlabel('Learning Rate', fontsize=20)
plt.ylabel('Region Counts', fontsize=20)
plt.grid(True)
# 显示图表
plt.savefig('teaser_lr.png')
