...
for net in nets:
    ...
    # 提取所需的列
    learning_rates = averaged_data[:, -2]  # 倒数第二列：learning rate
    ...
    # 绘制线条
    plt.plot(mapped_learning_rates, regions, label=f'{net}', marker=notations[cnt], markersize=10, linewidth=3)
...
y_min, y_max = 2.6,4.2
y_ticks = np.linspace(y_min, y_max, 5)  # 五个等距刻度
y_ticks_rounded = np.round(y_ticks, 1)  # 将刻度值四舍五入到两位小数
plt.yticks(y_ticks_rounded, fontsize=18)
