...
for dataset in datasets:
    ...
    for index in indexs:
        ...
        plt.plot(x_range, regression_line(x_range), label=None, linestyle='--',linewidth=3.5, color='blue')
        # 绘制散点图
        plt.scatter(x_current, y_current, color='blue')
        plt.xlabel('Region Counts',fontsize=28)
        plt.ylabel('Generalization Gap',fontsize=28)
        plt.grid(True)
        plt.tick_params(axis='both', which='major', labelsize=28)
        if dataset == 'mixup':
            plt.title(f'Correlation of Mixup: {correlation:.2f}', fontsize=28)
        else:
            plt.title(f'Correlation of Random Crop and Flip: {correlation:.2f}', fontsize=28)
